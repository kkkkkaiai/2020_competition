...
# 定义倾倒角度
turn = [1.769-0.5695, 1.923-0.5695, 2.2584-0.5695]#right
# 定义两个列表
...
def gripper_to_get_the_cup_2():  # 返回手臂看到的位置
    ...
    cup_base_tf = np.array(0.586, -0.34 , 0.042)
    #bottle_base_tf = from_head_to_cup_above(7)
    bottle_base_tf = np.array(0.646, 0.036, 0.328)
...
def taskC():
    ...
    #cup_base_tf = [0.586, -0.34, 0.042]
    # bottle_base_tf = from_head_to_cup_above(7)
    #bottle_head_tf = np.array(0.646, 0.036, 0.328)

    #pick_pose0 = []
    limb = 'right'
    # 定义倾倒角度
    #turn = [0.15, 0.26, 0.4]
    turn = [1.82 - 0.5695, 1.95 - 0.5695, 2.08 - 0.5695, 2.14 - 0.5695]  # right
    hand_bias_x = 0
    hand_bias_y = 0
    # bias = [-0.03, 0.07, 0.08]
    limb1 = 'left'
    ...
    don_move = "yes"
    # print('before wile')
    bottle_hight = [0.01,0.03,0.04, 0.05]
    for n in range(1):
        bottle_cup = [0.627 - 0.7 + 0.02, 0.081 - 0.044 - 0.015, bottle_hight[n]]  # right
        # print('wile')
        # bottle_base_tf00 = from_head_to_cup_above(7)
        bottle_base_tf00 = [0.646, 0.036, 0.164]
        ...
        pick_pose[0:3] = [bottle_pos[0] + bottle_cup[0], bottle_pos[1] + bottle_cup[1], bottle_pos[2] + bottle_cup[2]]
        robot.baxter_ik_move(limb, pick_pose[0:3], pick_pose[3:7])
        angel = baxter_interface.Limb(limb).joint_angles()
        angel[limb + '_w2'] = angel[limb + '_w2'] + turn[n]  # fall
