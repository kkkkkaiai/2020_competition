...
def baxter_ik_move(limb, rpy_pose):
    ...
    rpy_pose = camera_to_base_tf(rpy_pose)
...
def camera_open(_hand_camera, resolution=(960, 600), exposure=-1, gain=-1):
...
def gripper_control(_gripper, value, offset=0):
    if offset == 0:
        if _gripper == 'left':
        ...
        if _gripper == 'left':
            current_left = left.position()
            left.command_position(current_left + offset)
        elif _gripper == 'right':
            current_right = right.position()
...
if __name__ == "__main__":
    ...
    # baxter_ik_move('left', np.array([-0.436535,0.130556,1.032]))
