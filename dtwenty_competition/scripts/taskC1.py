...
def from_hand(position, classify='cup'):
    if (position[1] >= 0):
        gripper = 'left'
        frame = left_frame
        if (classify == 'cup'):
...
def gripper_to_get_the_cup_2():#返回手臂看到的位置
    ...
    cup_hand_tf = from_hand(cup_base_tf, 'cup')
    bottle_hand_tf = from_hand(bottle_base_tf, 'bottle')
    # bottle_base_tf = np.array([0.6,0.187,0.07])
    print("cup_hand_tf", cup_hand_tf)
    print("bottle_hand_tf", bottle_hand_tf)
    return cup_hand_tf, bottle_hand_tf
...
def if_pour(pour_list):  # 判断是否是倾倒位置
    if abs(pour_list[-1] - pour_list[-3]) > 0.03 or abs(pour_list[-2] - pour_list[-4]) > 0.03:
        ifpour = 1
    else:
        ifpour = 0
    return ifpour
...
def taskC():
    ...
    cup_base_tf, bottle_base_tf = gripper_to_get_the_cup_2()
    pick_pose0 = []
    ...
    while (ifmove):
        ...
        bottle_pos = [bottle_base_tf00[0], bottle_base_tf00[1], 0.2]
        print("bottle_pos", bottle_pos)
        move_list = move_list + bottle_pos[0:2]
        ifmove = if_move(move_list)
        if ifmove == 1:#jiancedaoyundong
            robot.baxter_ik_move(limb, [bottle_pos[0] + bias[0], bottle_pos[1] + bias[1], 0.2], pick_pose[3:7])
            continue
            ...
            print("ifpour", ifpour)
            if ifpour == 1:
                pick_pose[0:3] = [bottle_pos[0] + bias[0], bottle_pos[1] + bias[1], 0.2]
