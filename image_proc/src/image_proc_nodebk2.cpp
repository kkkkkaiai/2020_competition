...
int MinArea_clr = 110, MaxArea_clr = 500;
...
public:
    ...
    bool imageProcessCallback(image_proc_msgs::ImageProcSrv::Request &req,
		...
		try
		{
			if(req.method == 0)
				...
				result = wristRecog(left_color_image, 1, 215);
			else if(req.method == 2)
				result = wristRecog(right_color_image, 0, 215);
			else if(req.method == 3)
				result = taskA_fun1(rgbmat,rgbd2); // multi recognition
			else if(req.method == 4)
				result = wristRecog(left_color_image, 1, 185);
			else if(req.method == 5)
				result = wristRecog(right_color_image, 0, 185);  // task B
			else
			{
				ROS_INFO("Request Method ERROR");
				return false;
			}
	...
	void findver_color(Mat colorImg, vector<Point2f> crosspcnt, Mat depImg, vector<float> &p)
	{
		Mat copyimg = colorImg.clone();
		Mat binaryimg = Mat::zeros(colorImg.rows, colorImg.cols, CV_8UC1);
			...
			for (int i = 0; i < contours.size(); i++)
			{
				...
				if (contours[i].size() > MinArea_clr && contours[i].size() < MaxArea_clr)
				{
					...
					for (int k = 0; k < crosspcnt.size(); k++)
					{
						...
						{
							//p.push_back(1);
							p.push_back(center.x);
							p.push_back(center.y);
							p.push_back(depImg.at<unsigned short int>(crosspcnt[k].y, crosspcnt[k].x));
...
#define hor_dep_max 35
	...
	vector<float> findhorizontal(Mat colorImg, Mat depImg)
	{
		vector<float> p;
		Mat copyimg = colorImg.clone();
		Mat binaryimg = Mat::zeros(colorImg.rows, colorImg.cols, CV_8UC1);
	...
	vector<float> find_stand_obj(Mat colorImg, Mat depobj)
	{
		vector<Point2f> crosspcnt;
		vector<float> pp;
		findvertical(depobj, crosspcnt);
		findver_color(colorImg, crosspcnt, depobj, pp);
		if (pp[0] == -1)
		{
	...
	vector<float> taskA_fun1(Mat colorImg, Mat depImg)
	{
		Mat depobj, depImgBG;
		...
		/***vertical***/
		/***the p of vertical object contains :
	 * p[0]-- 0 or 1. 0 means no vertical objects
	 * p[1]-p[3]--xyz
	 * p[4]--class of the objects (from 0 to 4 : column,cuboid[w=h],cuboid[w!=h],six-sided cylinder)
	 * **/
		p = find_stand_obj(colorImg, depobj);
		if (p[0] == -1)
		{

			/***horizontal***/
			/***the p of horizontal object contains :
		 * p[0]-- 0 or 1. 0 means no vertical objects
		 * p[1]-p[3]--xyz
		 * **/
			p = findhorizontal(colorImg, depobj);
			if (p[0] == -1)
			{
				cout << "no horizontal objects!" << endl;
				cout<< "grip process end"<<endl;
			}
			else
			{
				cout<< "grip method = horizontal"<<endl;
			}
			
		}
		else
		{
			cout<< "grip method = vertical"<<endl;
		}
		
		p[2] = float(depImg.at<unsigned short int>(p[1], p[0])) * 0.001;
		p[0] = -(p[0] - camera_cx) * p[2] / camera_fx;
		p[1] = (p[1] - camera_cy) * p[2] / camera_fy;
